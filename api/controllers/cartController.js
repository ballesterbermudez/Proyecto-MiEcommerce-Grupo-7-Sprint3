...
const cartController = {
  ...
  modifyCart: (req, res) => {
    ...
    newCart.forEach(product=>persistance.inster(Cart, {id_product: product.id, id_user: req.params.id}))
    const modifiedCart = persistance.getCartByUserID(req.params.id);
    res.json({msg: 'Carrito modificado', user: req.params.id, cart: modifiedCart })
  }
};
